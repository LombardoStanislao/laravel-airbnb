...
class PaymentController extends Controller
{
    ...
    public function checkout($apartment_id, Request $request) {
        ...
        if ($result->success) {
            ...
            return redirect()->route('admin.apartments.show', ['apartment' => $apartment_id])->with('success-transaction', 'La transazione è avvenuta con successo. Ora il tuo appartamento è sponsorizzato.');
        } else {
            $new_payment->accepted = 0;
            $new_payment->save();

            return back()->with('error-transaction', 'La transazione è fallita.');
        }
